...
if (class_exists('PMProGateway_Razorpay')) {
	if (isset($_POST['confirm']) && $_POST['confirm'] == '1') {
		if (isset($_POST['pmpro_cancel-nonce']) && wp_verify_nonce($_POST['pmpro_cancel-nonce'], 'pmpro_cancel-nonce')) {
			$user_id = get_current_user_id();
			$old_level_id = isset($_POST['levelstocancel']) ? intval($_POST['levelstocancel']) : 0;

			// Instantiate the PMProGateway_Razorpay class
			$razorpay = new PMProGateway_Razorpay();

			if (method_exists($razorpay, 'razorpay_cancel_subscription')) {
				$result = $razorpay->razorpay_cancel_subscription($user_id, $old_level_id);
				if ($result) {
					wp_redirect(pmpro_url('account'));
					exit;
				} else {
					wp_die('Error cancelling the subscription.');
				}
			} else {
				wp_die('Cancellation method not found.');
			}
		}
	}
}
